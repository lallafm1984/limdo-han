...
- 새 화면 근거에서 가장 중요한 불편 하나 또는 구체적인 미검증 흐름 하나로 다음 루프 준비
...
작업자는 `AGENTS.md` 중지 조건에 도달하면 루프를 `차단`으로 기록하고 종료한다. 완료 루프는 최종 검증, 새 아이 대리 QA, 발견 기록, 다음 루프 하나 준비, 체크포인트 커밋, `git push origin HEAD` 성공까지 마친 뒤 인계한다. 일반 push가 실패하면 정확한 오류를 기록하고 force push하지 않는다. QA에서 제품 불편을 찾지 못하면 구체적인 미검증 흐름을 다음 QA 탐색 루프로 만들며 사람 검토를 기다리지 않는다.
