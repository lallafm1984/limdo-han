...
grep -q 'codex exec --ephemeral --json --sandbox danger-full-access' CLI_AUTOMATION.md || fail "fresh-session command contract missing"
...
grep -q 'CODEX_LOOP_SANDBOX:-danger-full-access' scripts/run-cli-loop.sh || fail "Android tooling sandbox mode missing"
