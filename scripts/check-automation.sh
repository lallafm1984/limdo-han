...
has_work_block="$(sed -n '/^has_work() {/,/^}/p' scripts/run-cli-loop.sh)"
grep -q 'status=' <<< "$has_work_block" || fail "감독자의 작업 판정이 루프 상태를 읽지 않음"
grep -q '차단|완료)' <<< "$has_work_block" || fail "차단·완료 상태 가드가 없음"
grep -q 'return 1' <<< "$has_work_block" || fail "차단·완료 상태에서 감독자가 종료하지 않음"
...
grep -q '^stabilize_display() {' scripts/ensure-emulator-ready.sh || fail "cold boot 회전 안정화 함수가 없음"
grep -q 'cold boot 뒤 user_rotation=1이 15회 안에 안정화되지 않음' scripts/ensure-emulator-ready.sh || fail "cold boot 회전 안정화 상한이 없음"
grep -q '^stabilize_display$' scripts/ensure-emulator-ready.sh || fail "에뮬레이터 준비가 회전 안정화 함수를 호출하지 않음"
