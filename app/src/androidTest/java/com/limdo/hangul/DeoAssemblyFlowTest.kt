...
class DeoAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun deoAssemblyRejectsVowelFirstThenWritesAndAdvances() {
        ...
        composeRule.onNode(hasContentDescription("더 조립 선택")).performClick()
        composeRule.waitUntil(3_000) { composeRule.onAllNodes(hasContentDescription("더 조립 미완성")).fetchSemanticsNodes().isNotEmpty() }
        captureEvidence("deo-start")
        composeRule.onNode(hasContentDescription("어 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("더 조립 미완성")).assertExists()
        captureEvidence("deo-wrong-order")
        composeRule.onNode(hasContentDescription("디귿 조각")).performClick()
        composeRule.onNode(hasContentDescription("어 모음 조각")).performClick()
        captureEvidence("deo-complete")
        composeRule.onNode(hasContentDescription("완성한 더 쓰기 시작")).performClick()
        captureEvidence("deo-writing")

        val canvas = composeRule.onNode(hasContentDescription("더를 4획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop224-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop224-iteration1-${file.name}").use { FileInputStream(it.fileDescriptor).use(FileInputStream::readBytes) }
        }
    }
}
