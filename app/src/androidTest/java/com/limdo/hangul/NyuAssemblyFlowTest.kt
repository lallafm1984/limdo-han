...
class NyuAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun nyuAssemblyRejectsVowelFirstThenWritesAndAdvancesToDa() {
        ...
        composeRule.onNode(hasContentDescription("뉴 조립 선택")).performClick()
        composeRule.waitUntil(3_000) { composeRule.onAllNodes(hasContentDescription("뉴 조립 미완성")).fetchSemanticsNodes().isNotEmpty() }
        captureEvidence("nyu-start")
        composeRule.onNode(hasContentDescription("유 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("뉴 조립 미완성")).assertExists()
        captureEvidence("nyu-wrong-order")
        composeRule.onNode(hasContentDescription("니은 조각")).performClick()
        composeRule.onNode(hasContentDescription("유 모음 조각")).performClick()
        captureEvidence("nyu-complete")
        composeRule.onNode(hasContentDescription("완성한 뉴 쓰기 시작")).performClick()
        captureEvidence("nyu-writing")

        val canvas = composeRule.onNode(hasContentDescription("뉴를 4획으로", substring = true) and hasClickAction())
        repeat(3) { canvas.performSemanticsAction(SemanticsActions.OnClick); composeRule.waitForIdle() }
        ...
        Thread.sleep(1_000)
        captureEvidence("nyu-success")
        composeRule.onNode(hasContentDescription("다음 글자로 이동")).performClick()
        composeRule.mainClock.autoAdvance = true
        composeRule.waitUntil(10_000) { composeRule.onAllNodes(hasContentDescription("다를 4획으로", substring = true)).fetchSemanticsNodes().isNotEmpty() }
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop220-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop220-iteration1-${file.name}").use { FileInputStream(it.fileDescriptor).use(FileInputStream::readBytes) }
        }
    }
}
