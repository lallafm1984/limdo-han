...
class NeuAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun neuAssemblyRejectsVowelFirstThenWritesAndAdvancesToDa() {
        ...
        composeRule.onNode(hasContentDescription("느 조립 선택")).performClick()
        composeRule.waitUntil(3_000) { composeRule.onAllNodes(hasContentDescription("느 조립 미완성")).fetchSemanticsNodes().isNotEmpty() }
        captureEvidence("neu-start")
        composeRule.onNode(hasContentDescription("으 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("느 조립 미완성")).assertExists()
        captureEvidence("neu-wrong-order")
        composeRule.onNode(hasContentDescription("니은 조각")).performClick()
        composeRule.onNode(hasContentDescription("으 모음 조각")).performClick()
        captureEvidence("neu-complete")
        composeRule.onNode(hasContentDescription("완성한 느 쓰기 시작")).performClick()
        captureEvidence("neu-writing")

        val canvas = composeRule.onNode(hasContentDescription("느를 2획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop221-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop221-iteration1-${file.name}").use { FileInputStream(it.fileDescriptor).use(FileInputStream::readBytes) }
        }
    }
}
