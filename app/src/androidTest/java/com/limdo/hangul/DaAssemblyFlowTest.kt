...
class DaAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun daAssemblyRejectsVowelFirstThenWritesAndAdvancesToRa() {
        ...
        composeRule.onNode(hasContentDescription("다 조립 선택")).performClick()
        composeRule.waitUntil(3_000) { composeRule.onAllNodes(hasContentDescription("다 조립 미완성")).fetchSemanticsNodes().isNotEmpty() }
        captureEvidence("da-start")
        composeRule.onNode(hasContentDescription("아 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("다 조립 미완성")).assertExists()
        captureEvidence("da-wrong-order")
        composeRule.onNode(hasContentDescription("디귿 조각")).performClick()
        composeRule.onNode(hasContentDescription("아 모음 조각")).performClick()
        captureEvidence("da-complete")
        composeRule.onNode(hasContentDescription("완성한 다 쓰기 시작")).performClick()
        captureEvidence("da-writing")

        val canvas = composeRule.onNode(hasContentDescription("다를 4획으로", substring = true) and hasClickAction())
        ...
        Thread.sleep(1_000)
        captureEvidence("da-success")
        composeRule.onNode(hasContentDescription("다음 글자로 이동")).performClick()
        composeRule.mainClock.autoAdvance = true
        captureEvidence("da-after-next")
        composeRule.waitUntil(10_000) { composeRule.onAllNodes(hasContentDescription("라를 5획으로", substring = true)).fetchSemanticsNodes().isNotEmpty() }
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop223-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop223-iteration1-${file.name}").use { FileInputStream(it.fileDescriptor).use(FileInputStream::readBytes) }
        }
    }
}
