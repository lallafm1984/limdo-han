...
class DiAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun diAssemblyRejectsVowelFirstThenWritesAndAdvances() {
        ...
        composeRule.onNode(hasContentDescription("디 조립 선택")).performClick()
        composeRule.waitUntil(3_000) { composeRule.onAllNodes(hasContentDescription("디 조립 미완성")).fetchSemanticsNodes().isNotEmpty() }
        captureEvidence("di-start")
        composeRule.onNode(hasContentDescription("이 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("디 조립 미완성")).assertExists()
        captureEvidence("di-wrong-order")
        composeRule.onNode(hasContentDescription("디귿 조각")).performClick()
        composeRule.onNode(hasContentDescription("이 모음 조각")).performClick()
        captureEvidence("di-complete")
        composeRule.onNode(hasContentDescription("완성한 디 쓰기 시작")).performClick()
        captureEvidence("di-writing")

        composeRule.mainClock.autoAdvance = false
        repeat(3) {
            composeRule.onNode(hasContentDescription("디를 3획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop231-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop231-iteration1-${file.name}").use { FileInputStream(it.fileDescriptor).use(FileInputStream::readBytes) }
        }
    }
}
