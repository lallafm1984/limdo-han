...
class DoAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun doAssemblyRejectsVowelFirstThenWritesAndAdvances() {
        ...
        composeRule.onNode(hasContentDescription("도 조립 선택")).performClick()
        composeRule.waitUntil(3_000) {
            composeRule.onAllNodes(hasContentDescription("도 조립 미완성")).fetchSemanticsNodes().isNotEmpty()
        }
        captureEvidence("do-start")
        composeRule.onNode(hasContentDescription("오 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("도 조립 미완성")).assertExists()
        captureEvidence("do-wrong-order")
        composeRule.onNode(hasContentDescription("디귿 조각")).performClick()
        composeRule.onNode(hasContentDescription("오 모음 조각")).performClick()
        captureEvidence("do-complete")
        composeRule.onNode(hasContentDescription("완성한 도 쓰기 시작")).performClick()
        captureEvidence("do-writing")

        val canvas = composeRule.onNode(hasContentDescription("도를 4획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop226-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop226-iteration1-${file.name}").use {
