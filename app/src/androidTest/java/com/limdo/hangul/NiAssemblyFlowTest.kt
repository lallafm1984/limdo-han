...
class NiAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun niAssemblyRejectsVowelFirstThenWritesAndAdvancesToDa() {
        ...
        composeRule.onNode(hasContentDescription("니 조립 선택")).performClick()
        composeRule.waitUntil(3_000) { composeRule.onAllNodes(hasContentDescription("니 조립 미완성")).fetchSemanticsNodes().isNotEmpty() }
        captureEvidence("ni-start")
        composeRule.onNode(hasContentDescription("이 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("니 조립 미완성")).assertExists()
        captureEvidence("ni-wrong-order")
        composeRule.onNode(hasContentDescription("니은 조각")).performClick()
        composeRule.onNode(hasContentDescription("이 모음 조각")).performClick()
        captureEvidence("ni-complete")
        composeRule.onNode(hasContentDescription("완성한 니 쓰기 시작")).performClick()
        captureEvidence("ni-writing")

        val canvas = composeRule.onNode(hasContentDescription("니를 2획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop222-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop222-iteration1-${file.name}").use { FileInputStream(it.fileDescriptor).use(FileInputStream::readBytes) }
        }
    }
}
