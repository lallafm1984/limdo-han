...
class DeuAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun deuAssemblyRejectsVowelFirstThenWritesAndAdvances() {
        ...
        composeRule.onNode(hasContentDescription("드 조립 선택")).performClick()
        composeRule.waitUntil(3_000) {
            composeRule.onAllNodes(hasContentDescription("드 조립 미완성")).fetchSemanticsNodes().isNotEmpty()
        }
        captureEvidence("deu-start")
        composeRule.onNode(hasContentDescription("으 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("드 조립 미완성")).assertExists()
        captureEvidence("deu-wrong-order")
        composeRule.onNode(hasContentDescription("디귿 조각")).performClick()
        composeRule.onNode(hasContentDescription("으 모음 조각")).performClick()
        captureEvidence("deu-complete")
        composeRule.onNode(hasContentDescription("완성한 드 쓰기 시작")).performClick()
        captureEvidence("deu-writing")

        composeRule.mainClock.autoAdvance = false
        repeat(3) {
            composeRule.onNode(hasContentDescription("드를 3획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop230-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop230-iteration1-${file.name}").use {
