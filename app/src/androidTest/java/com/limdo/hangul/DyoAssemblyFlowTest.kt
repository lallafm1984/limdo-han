...
class DyoAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun dyoAssemblyRejectsVowelFirstThenWritesAndAdvances() {
        ...
        composeRule.onNode(hasContentDescription("\uB434 조립 선택")).performClick()
        composeRule.waitUntil(3_000) {
            composeRule.onAllNodes(hasContentDescription("\uB434 조립 미완성")).fetchSemanticsNodes().isNotEmpty()
        }
        captureEvidence("dyo-start")
        composeRule.onNode(hasContentDescription("요 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("\uB434 조립 미완성")).assertExists()
        captureEvidence("dyo-wrong-order")
        composeRule.onNode(hasContentDescription("\uB514\uADE3 조각")).performClick()
        composeRule.onNode(hasContentDescription("요 모음 조각")).performClick()
        captureEvidence("dyo-complete")
        composeRule.onNode(hasContentDescription("완성한 \uB434 쓰기 시작")).performClick()
        captureEvidence("dyo-writing")

        val canvas = composeRule.onNode(hasContentDescription("\uB434를 5획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop227-iteration1").apply { mkdirs() }
        ...
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop227-iteration1-${file.name}").use {
