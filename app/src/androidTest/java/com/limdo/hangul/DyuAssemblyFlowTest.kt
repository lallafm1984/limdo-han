...
class DyuAssemblyFlowTest {
    @get:Rule val composeRule = createAndroidComposeRule<MainActivity>()

    @Test fun dyuAssemblyRejectsVowelFirstThenWritesAndAdvances() {
        ...
        composeRule.onNode(hasContentDescription("듀 조립 선택")).performClick()
        composeRule.waitUntil(3_000) {
            composeRule.onAllNodes(hasContentDescription("듀 조립 미완성")).fetchSemanticsNodes().isNotEmpty()
        }
        captureEvidence("dyu-start")
        composeRule.onNode(hasContentDescription("유 모음 조각")).performClick()
        composeRule.onNode(hasContentDescription("듀 조립 미완성")).assertExists()
        captureEvidence("dyu-wrong-order")
        composeRule.onNode(hasContentDescription("디귣 조각")).performClick()
        composeRule.onNode(hasContentDescription("유 모음 조각")).performClick()
        captureEvidence("dyu-complete")
        composeRule.onNode(hasContentDescription("완성한 듀 쓰기 시작")).performClick()
        captureEvidence("dyu-writing")

        val canvas = composeRule.onNode(hasContentDescription("듀를 5획으로", substring = true) and hasClickAction())
    ...
    private fun captureEvidence(name: String) {
        ...
        val evidenceDir = File(instrumentation.targetContext.getExternalFilesDir(null), "loop229-iteration1").apply { mkdirs() }
        ...
        uiAutomation.executeShellCommand("dumpsys window").use { descriptor ->
            FileInputStream(descriptor.fileDescriptor).use { input -> focus.outputStream().use { input.copyTo(it) } }
        }
        hierarchy.writeText(composeRule.onRoot(useUnmergedTree = true).printToString())
        listOf(screenshot, focus, hierarchy).forEach { file ->
            uiAutomation.executeShellCommand("cp ${file.absolutePath} /data/local/tmp/loop229-iteration1-${file.name}").use {
